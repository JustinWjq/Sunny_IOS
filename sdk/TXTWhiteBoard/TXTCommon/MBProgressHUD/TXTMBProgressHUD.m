...
#define MBMainThreadAssert() NSAssert([NSThread isMainThread], @"TXTMBProgressHUD needs to be accessed on the main thread.");

CGFloat const TXTMBProgressMaxOffset = 1000000.f;
...
@interface TXTMBProgressHUDRoundedButton : UIButton
@end


@implementation TXTMBProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    TXTMBProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    TXTMBProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (TXTMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            return (TXTMBProgressHUD *)subview;
        }
    }
    return nil;
}
...
- (void)commonInit {
    ...
    _animationType = TXTMBProgressHUDAnimationFade;
    _mode = TXTMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(TXTMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == TXTMBProgressHUDAnimationZoom) {
        type = animatingIn ? TXTMBProgressHUDAnimationZoomIn : TXTMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == TXTMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == TXTMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == TXTMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == TXTMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    TXTMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<TXTMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    TXTMBBackgroundView *backgroundView = [[TXTMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = TXTMBProgressHUDBackgroundStyleSolidColor;
    ...
    TXTMBBackgroundView *bezelView = [TXTMBBackgroundView new];
    ...
    UIButton *button = [TXTMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[TXTMBRoundProgressView class]];

    TXTMBProgressHUDMode mode = self.mode;
    if (mode == TXTMBProgressHUDModeIndeterminate) {
    ...
    else if (mode == TXTMBProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[TXTMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == TXTMBProgressHUDModeDeterminate || mode == TXTMBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[TXTMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == TXTMBProgressHUDModeAnnularDeterminate) {
            [(TXTMBRoundProgressView *)indicator setAnnular:YES];
        }
    } 
    else if (mode == TXTMBProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == TXTMBProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[TXTMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[TXTMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[TXTMBRoundProgressView class]]) {
        TXTMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TXTMBRoundProgressView appearanceWhenContainedIn:[TXTMBProgressHUD class], nil];
#else
        appearance = [TXTMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[TXTMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((TXTMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((TXTMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[TXTMBBarProgressView class]]) {
        TXTMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [TXTMBBarProgressView appearanceWhenContainedIn:[TXTMBProgressHUD class], nil];
#else
        appearance = [TXTMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[TXTMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((TXTMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((TXTMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    TXTMBBackgroundView *bezelView = self.bezelView;
...
- (void)setMode:(TXTMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == TXTMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation TXTMBRoundProgressView
...
@implementation TXTMBBarProgressView
...
@implementation TXTMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = TXTMBProgressHUDBackgroundStyleBlur;
            ...
            _style = TXTMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(TXTMBProgressHUDBackgroundStyle)style {
    if (style == TXTMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = TXTMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    TXTMBProgressHUDBackgroundStyle style = self.style;
    if (style == TXTMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == TXTMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation TXTMBProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [TXTMBProgressHUD allHUDsForView:view];
    for (TXTMBProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable TXTMBProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    TXTMBBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == TXTMBProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    MBMainThreadAssert();
    self.backgroundView.style = TXTMBProgressHUDBackgroundStyleSolidColor;
...
@implementation TXTMBProgressHUDRoundedButton
